...
namespace docker_port
{
    class Program
    {
        static string DOCKER_TEMP_FILE_LOCATION = Path.GetTempFileName();
        ...
        private static async Task HandleContainerInjection(DockerClient client, JToken parsedBuffer)
        {
            ...
            try
            {
                matchingPorts = output.Result.NetworkSettings.Ports.Keys.Select(s => int.Parse(s.Split("/")[0])).Intersect(ports).ToList();
        ...
        private static void ExtractDockerPorts(string[] lines, List<int> ports)
        {
            foreach (var line in lines)
            {
                try
                {
                    ports.Add(Int32.Parse(Regex.Match(line, @"\:([0-9]+) ").Groups[0].ToString().Replace(":", "")));
                }
                catch (Exception)
                {
